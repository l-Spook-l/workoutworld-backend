...
from src.core.exceptions.custom_exceptions import NotFoundError
...
class WorkoutRepository:
    ...
    async def update_workout(self, workout_id: int, data: WorkoutUpdate):
        query = update(Workout).where(Workout.id == workout_id).values(**data.model_dump(exclude_none=True))
    ...
    async def get_workouts(
        ...
        if name:
            query = query.where(Workout.name.ilike(f"%{name}%"))
        if difficulty:
            query = query.where(Workout.difficulty.in_(difficulty))
        if is_public is not None:
            query = query.where(Workout.is_public == is_public)
    ...
    async def add_user_workout_association(self, user_id, workout_id):
        ...
        query_workout = select(Workout).where(Workout.id == workout_id)
    ...
    async def get_one_workout(self, workout_id: int) -> Workout | None:
        query = select(Workout).where(Workout.id == workout_id).options(
    ...
    async def get_active_workout(self, workout_id: int, user_id: int):
        association_query = select(added_workouts_association).where(
            added_workouts_association.c.workout_table == workout_id,
            added_workouts_association.c.user_table == user_id)
    ...
    async def get_user_added_workouts(
        ...
        query = select(Workout)
        query = query.join(added_workouts_association).where(added_workouts_association.c.user_table == user_id)
    ...
    async def count_user_added_workouts(
        ...
        query = (
            ...
            .where(added_workouts_association.c.user_table == user_id)
        )
        if name:
            query = query.where(Workout.name.ilike(f"%{name}%"))
        if difficulty:
    ...
    async def get_exercises_by_workout_id(self, workout_id: int):
        query = await self.session.execute(select(Exercise).where(Exercise.workout_id == workout_id))
    ...
    async def get_photos_exercise_by_id(self, exercise_id: int):
        photos_exercise = await self.session.execute(
            select(Exercise_photo).where(Exercise_photo.exercise_id == exercise_id))
...
class ExerciseRepository:
    ...
    async def get_photos_by_ids(self, photo_ids: list[int]):
        query = select(Exercise_photo).where(Exercise_photo.id.in_(photo_ids))
        result = await self.session.execute(query)
    ...
    async def delete_photos_by_ids(self, photo_ids: list[int]):
        await self.session.execute(delete(Exercise_photo).where(Exercise_photo.id.in_(photo_ids)))
...
class SetRepository:
    ...
    async def get_sets(self, user_id: int, exercise_ids: list[int]):
        query = select(Set).where(Set.exercise_id.in_(exercise_ids), Set.user_id == user_id).order_by(Set.id)
        result = await self.session.execute(query)
        sets = result.mappings().all()
        return sets

    async def update_set(self, set_id: int, update_data: SetUpdate):
        query = update(Set).where(Set.id == set_id).values(**update_data.model_dump(exclude_none=True))
        await self.session.execute(query)

    async def delete_set(self, exercise_id: int, user_id: int):
        query = delete(Set).where(Set.exercise_id == exercise_id, Set.user_id == user_id)
        await self.session.execute(query)
